...
def get_predicted_odds(current_date_time, session_id):
    ...
    if website == predictz:
        ...
        for i in range(10):
            home_odds = percentage_to_decimal_odds(int(home_prob[i + 1]))
            draw_odds = percentage_to_decimal_odds(int(draw_prob[i + 1]))
            away_odds = percentage_to_decimal_odds(int(away_prob[i + 1]))
            ...
            DBS.insert_forebet_odds(current_date_time, home_odds, draw_odds, away_odds, session_id, home_teams[i], away_teams[i],current_date_time )
# NOTE - MATCH DATE IS WRONG